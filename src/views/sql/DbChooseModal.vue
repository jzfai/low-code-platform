<template>
  ...
  >
    <div class="mb-30px">
      <el-select v-model="currentRow.filterTableName" filter filterable placeholder="组件类型">
        <el-option
            v-for="(item, index) in chooseTableArr"
            :key="index"
            :label="`${item.tableName}(${item.tableDesc})`"
    ...
    <div>
      <el-select v-model="currentRow.filterColumnName" filter filterable placeholder="组件类型">
        <el-option
