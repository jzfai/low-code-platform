...
export const dillRequestBody=(requestBody,definitions)=>{
  ...
  if(requestBody?.content["application/json"]?.schema?.$ref){
    ...
    Object.keys(schemasItem).forEach(key=>{
      paramsArr.push({name:key,description:schemasItem[key].description,required:requiredKeys?requiredKeys.includes(key):false})
    })
  }else{
    //取key
  ...
  if(responses[200]?.schema?.$ref){
    ...
    Object.keys(schemas).forEach(key=>{
      if(schemas[key]?.items?.$ref||schemas[key]?.$ref){
        ...
        if(schemasItem){
          Object.keys(schemasItem).forEach(key=>{
            responsesArr.push({name:key,description:schemasItem[key].description,required:requiredKeys?requiredKeys.includes(key):false})
          })
        }
      }
    })
  ...
  if(responses[200]?.content&&responses[200].content["*/*"]?.schema?.$ref){
    ...
    Object.keys(schemas).forEach(key=>{
      if(schemas[key]?.items?.$ref||schemas[key]?.$ref){
        ...
        const requiredKeys=components.schemas[define].required
        Object.keys(schemasItem).forEach(key=>{
          responsesArr.push({name:key,description:schemasItem[key].description,required:requiredKeys?.includes(key)})
        })
      }
    })
  }else{
